dnl This part is adapted from the postgresql cvs
dnl (available at http://www.postgresql.org)
dnl and was slightly modified to fit wzdftpd.

dnl This file defines new macros to process configure command line
dnl arguments, to replace the brain-dead AC_ARG_WITH and AC_ARG_ENABLE.
dnl The flaw in these is particularly that they only differentiate
dnl between "given" and "not given" and do not provide enough help to
dnl process arguments that only accept "yes/no", that require an
dnl argument (other than "yes/no"), etc.
dnl 
dnl The point of this implementation is to reduce code size and
dnl redundancy in configure.in and to improve robustness and consistency
dnl in the option evaluation code.
