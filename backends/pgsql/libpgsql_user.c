...
int wpgsql_mod_user(uid_t uid, wzd_user_t * user, unsigned long mod_type)
{
...

  {
    ...
    if (_wzd_run_update_query(query, 2048, "INSERT INTO users (username,userpass,rootpath,uid,creator,flags,max_idle_time,max_ul_speed,max_dl_speed,num_logins,ratio,user_slots,leech_slots,perms,credits) VALUES ('%s','%s','%s',nextval('users_uid_seq'),'%u','%s',%u,%lu,%lu,%u,%u,%u,%u,CAST (X'%lx' as integer),% " PRIu64 ")",
